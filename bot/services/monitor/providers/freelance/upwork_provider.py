...
class UpworkFreelanceProvider:
    platform = "Upwork"
    search_url = "https://www.upwork.com/nx/search/jobs/?q=software%20development&sort=recency"
    ...
    def _parse(self, html: str) -> list[FreelanceOpportunity]:
        ...
        for match in re.finditer(r'href="(?P<url>/jobs/[^"]+)"[^>]*>(?P<title>[^<]{8,180})<', html):
            url = "https://www.upwork.com" + unescape(match.group("url"))
    ...
    def _item(self, title: str, url: str) -> FreelanceOpportunity:
        ...
        return {
            "title": title,
            "client_or_company": "Cliente Upwork",
